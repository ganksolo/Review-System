...
async def trigger_analysis(trade_id: UUID, db: AsyncSession = Depends(get_db)):
    """手动触发单条交易的 LLM 分析。"""
    result = await trade_analyzer.analyze_trade(db, trade_id)
    await db.commit()
...
async def batch_analyze(
    req: BatchAnalyzeRequest, db: AsyncSession = Depends(get_db)
):
    """批量触发多条交易的 LLM 分析（并行 + 限流）。"""
    results = await trade_analyzer.analyze_batch(
        db, req.trade_ids, req.max_concurrent
    )
    await db.commit()
